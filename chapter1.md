...
**以管理员身份运行**VS（否则install时会失败），打开bulid文件夹下的eigen.sln工程，待加加完文件后，生成ALL\_BUILD(Debug和 Release均要生成)，完成后生成INSTALL(Debug and Release)。可以看到eigen安装路径中出现include文件夹。最后在环境变量中建立EIGEN\_ROOT变量，值为eigen的安装路径。
